...
#dpm($field->options['label']);
  ...
  if ($field->options['label']  == 'Status') {
    ...
    $result=$w->arguments('inspect', $row->nid, 0);   // get status

    if (isset($result['webfact_arguments'][0]['#status'])) {
      $output = $result['webfact_arguments'][0]['#status'];;
    }
    else {
      $output = 'unknown';
      watchdog('webfact', "status field: unknown for node $row->nid");
    }
    //print $output; 
    print "<div class=website-status>";
    if ( ($output=='running') || ($output=='stopped') ) {
